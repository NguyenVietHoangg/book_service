...
var ENVS_USE = ['development', 'development'];
...
function _sleep() {
  _sleep = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(fn) {
    var _len,
      args,
      _key,
      _args = arguments;
...
con.connect(function (err) {
  ...
  if (backupdb) {
    ...
    var sqlbackup = "mysqldump -u ".concat(setting.username, " -p'").concat(setting.password, "' ").concat(setting.database, " > ").concat(filebackup);
    var sqlbackupInSecond = "mysqldump -u ".concat(setting.username, " -p'").concat(setting.password, "' ").concat(setting.database, " > ").concat(filebackupInSecond);
  } else {
    ...
    con.query("DROP DATABASE IF EXISTS " + setting.database, function (err) {
      ...
      con.query("CREATE DATABASE " + setting.database + " CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;", function (err) {
        ...
        if (process.env.IS_RESTORE_DB && process.env.IS_RESTORE_DB === '1') {
          setTimeout(function () {
            var sqlrestore = "mysql -u ".concat(setting.username, " -p'").concat(setting.password, "' ").concat(setting.database, " < ").concat(filebackup);
