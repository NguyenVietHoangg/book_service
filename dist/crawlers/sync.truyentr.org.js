  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(url) {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        ...
        case 17:
          result = selector('.col-main-content');
          storyName = (0, _common.clearUnicode)(result.find('.book-head h1.title a').text());
          images = result.find('.sstbcover').attr('src');
          author = (0, _common.clearUnicode)(selector(result.find('.info p')[0]).text().replace('Tác giả:', ''));
          genre = (0, _common.clearUnicode)(selector(result.find('.info p')[1]).text().replace('Thể loại:', ''));
          resource = (0, _common.clearUnicode)((selector(result.find('.info p')[2]).text() + '-' + PREFIX_SOURCE).replace('Nguồn:', '').replace('Trạng thái:', ''));
          crawlerFrom = PREFIX_SOURCE;
          state = selector(result.find('.info div')[3]).text().replace('Trạng thái:', '');
          shortDescription = (0, _common.clearUnicode)(result.find('.book-desc').html());
          rate = 9.1;
          paginate = result.find('.pagination.pc .pagination.pagination-sm a').length ? result.find('.pagination.pc .pagination.pagination-sm a').length : 1;
          textfinalPaginate = selector(result.find('.pagination.pagination-sm a')[paginate - 1]).text();
          linkPageChapter = null;
