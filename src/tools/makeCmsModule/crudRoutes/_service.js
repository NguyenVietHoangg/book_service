...
export const get_table_generate_camel_case_List = async (conds = [], page = 1, opts = []) => {
  ...
  let { count, rows } = await Model._table_generate_.findAndCountAll({
    where,
    include,
    offset,
    limit: LIMIT
...
export const get_table_generate_camel_case_DetailById = async (id) => {
  if (!id) {
    return null
  }
  let where = {
  ...
  const row = await Model._table_generate_.findOne({
    where,
    include: [
      /**
       * include example width format
  ...
  if (row_table_generate_ && row._table_generate_.length) {
    row.dataValues._table_generate_.Ids = row._table_generate_.map(item => item.id)
  }
  return row
}

export const save_table_generate_camel_case_DetailById = async (id, data) => {
  let item = null
  if (!id || id ==  'new') {
    // kiêm tra tồn tại của bản ghi nháp
    item = await Model._table_generate_.findOne({
      where: {
        name: {
          [Op.eq]: null
        }
    ...
    if (!item) {
      item = await Model._table_generate_.create(Model.setupInsert({}))
    }
    id = item.id
  } else {
    item = await Model._table_generate_.findOne({
    ...
    await Model._table_generate_.update(Model.setupUpdate(data), {
      where: {id: id}
    })
  }
