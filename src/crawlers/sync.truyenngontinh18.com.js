...
const PREFIX_SOURCE = 'truyenngontinh18.com'
...
const getStoryDetail = async (url=null, storyId=null) => {
  ...
  const linkPageAjax = 'https://truyenngontinh18.com/wp-admin/admin-ajax.php'
  const storyIdCrawler = selector('#wrap').find('#id_post').val()
  ...
  const linkPageChapter = paginate===1?url:(!textfinalPaginate.includes('Cuối')) ? selector(result.find('.pagination.pagination-sm a')[paginate-1]).attr('href') : selector(result.find('.pagination.pagination-sm a')[paginate-1]).attr('href')
  ...
  for (let i=startI; i<=totalPage; i++) {
    ...
    const listLinks = selectorChapter('.col-xs-12 .list-chapter').find('a');
...
const getChapter = async (link, storyId, slugStory, chapterOrder=null) => {
  ...
  let chapterName = clearUnicode(selector('#wrap .row .chapter-title').text())
