...
const getStoryDetail = async (url, storyId=null) => {
  ...
  try {
    ...
    // selector = await config.configNodejsLoad(url)
  } catch (error) {
    console.log('error', error)
    return false
  }
  const result = selector('#wrap')
  const storyName = clearUnicode(result.find('.name.book h3').text())
  console.log(storyName)
  return;
  const images = '';
  console.log(selector(result.find('.thong-tin-info div [itemprop="author"]')[0]).text());
  return;
  const author = clearUnicode((selector(result.find('.thong-tin-info div [itemprop="author"]')[0]).text()).replace('Tác giả:', ''))
  ...
  Model.chapter.bulkCreate(chapters, { ignoreDuplicates : true});
  setTimeout(function() {
