...
const PREFIX_SOURCE = 'vi.blogtamsu.com'

const getStoryDetail = async (url, storyId=null) => {
  if (!url) {
    url = 'https://vi.blogtamsu.com/truyen/anh-chang-khong-ket-hon-lay-em-di/'
    return;
  }
  ...
  for(let i = startI; i <= totalPage; i++) {
    const endpoint = 'https://vi.blogtamsu.com/truyen/'+slugTmp+'/trang-'+i
    let crawler = null
    try {
  ...
  console.log('===== Xong getStoryDetail vi.blogtamsu.com !!!!!!!!!!!!!!: '+url)
  return true
}
export default {
  getStoryDetail
