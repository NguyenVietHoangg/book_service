...
export const getCollectionList = async (conds = [], page = 1, opts = []) => {
  ...
  let { count, rows } = await Model.collection.findAndCountAll({
    where,
    include,
    offset,
    limit: LIMIT
...
export const getCollectionDetailById = async (id) => {
  ...
  if (row.categories && row.categories.length) {
    row.dataValues.collectionIds = row.categories.map(item => item.id)
  }
  return row
}

export const saveCollectionDetailById = async (id, data) => {
  let item = null
  if (!id || id ==  'new') {
    // kiêm tra tồn tại của bản ghi nháp
    item = await Model.collection.findOne({
      where: {
        name: {
    ...
    if (!item) {
      item = await Model.collection.create(Model.setupInsert({}))
    }
    id = item.id
  } else {
    item = await Model.collection.findOne({
    ...
    await Model.collection.update(Model.setupUpdate(data), {
      where: {id: id}
    })
  }
