...
export const getundefinedList = async (conds = [], page = 1, opts = []) => {
  ...
  let { count, rows } = await Model.testx.findAndCountAll({
    where,
    include,
    offset,
    limit: LIMIT
...
export const getundefinedDetailById = async (id) => {
  ...
  const row = await Model.testx.findOne({
    where,
    include: [
      /**
       * include example width format
  ...
  if (rowtestx && row.testx.length) {
    row.dataValues.testx.Ids = row.testx.map(item => item.id)
  }
  return row
}
...
export const saveundefinedDetailById = async (id, data) => {
  ...
  if (!id || id ==  'new') {
    ...
    item = await Model.testx.findOne({
      where: {
        name: {
          [Op.eq]: null
        }
    ...
    if (!item) {
      item = await Model.testx.create(Model.setupInsert({}))
    }
    id = item.id
  } else {
    item = await Model.testx.findOne({
    ...
    await Model.testx.update(Model.setupUpdate(data), {
      where: {id: id}
    })
  }
