...
export const getPermissionList = async (data) => {
  try {
    ...
    Model.permission.hasMany(Model.grouprole_has_permission, {
      foreignKey: "permissionId",
    });
    Model.grouprole_has_permission.belongsTo(Model.permission, {
      foreignKey: "permissionId",
    });
...
export const addRoleHasPermission = async (data) => {
  try {
    const role = await Model.grouprole_has_permission.create(data.data);
    return role;
  } catch (error) {
    return false;
  }
};
export const deleteRoleHasPermission = async (data) => {

  try {
    const result = await Model.grouprole_has_permission.destroy({
      where: { groupId: data.data.groupId, permissionId: data.data.permissionId },
    });
    return result;
  } catch (error) {
    return false;
