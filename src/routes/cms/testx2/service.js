...
export const getundefinedList = async (conds = [], page = 1, opts = []) => {
  ...
  let { count, rows } = await Model.testx2.findAndCountAll({
    where,
    include,
    offset,
    limit: LIMIT
...
export const getundefinedDetailById = async (id) => {
  ...
  const row = await Model.testx2.findOne({
    where,
    include: [
      /**
       * include example width format
  ...
  if (rowtestx2 && row.testx2.length) {
    row.dataValues.testx2.Ids = row.testx2.map(item => item.id)
  }
  return row
}
...
export const saveundefinedDetailById = async (id, data) => {
  ...
  if (!id || id ==  'new') {
    ...
    item = await Model.testx2.findOne({
      where: {
        name: {
          [Op.eq]: null
        }
    ...
    if (!item) {
      item = await Model.testx2.create(Model.setupInsert({}))
    }
    id = item.id
  } else {
    item = await Model.testx2.findOne({
    ...
    await Model.testx2.update(Model.setupUpdate(data), {
      where: {id: id}
    })
  }
